...
// ─────────────────────────────────────────────────────────
//  CARD JS (injected into student page)
// ─────────────────────────────────────────────────────────
const CARD_JS = `
function getLastSeries(bdg){
  if(!bdg)return null;
...
function card(e,isW){
  ...
  const hasSaved=sv.series||sv.reps||sv.carga;
  const lastSeries=getLastSeries(e.bdg);
  const hint=lastSeries?'<div class="inline-reg-hint">solo registrar las últimas series: <strong>'+lastSeries+'</strong></div>':'';
  const inlineReg=(!isW&&e.se)?
    '<div class="inline-reg" onclick="event.stopPropagation()">'+
      '<div class="inline-reg-lbl">Mi registro</div>'+hint+
      '<div class="inline-reg-row">'+
        '<div class="ireg-f"><label>Series</label><input type="number" id="rs-'+e.id+'" value="'+(sv.series||'')+'" placeholder="'+(e.se||'—')+'" min="0" class="'+(sv.series?'has-value':'')+'" oninput="markDirty(\''+e.id+'\')"></div>'+
        '<div class="ireg-f"><label>Reps</label><input type="number" id="rr-'+e.id+'" value="'+(sv.reps||'')+'" placeholder="—" class="'+(sv.reps?'has-value':'')+'" oninput="markDirty(\''+e.id+'\')"></div>'+
        '<div class="ireg-f"><label>Carga</label><input type="text" id="rc-'+e.id+'" value="'+(sv.carga||'')+'" placeholder="kg" class="'+(sv.carga?'has-value':'')+'" oninput="markDirty(\''+e.id+'\')"></div>'+
      '</div>'+
      '<div class="inline-reg-row two-col">'+
        '<div class="ireg-f"><label>Sensación</label><input type="text" id="rf-'+e.id+'" value="'+(sv.feeling||'')+'" placeholder="bien / pesado..." oninput="markDirty(\''+e.id+'\')"></div>'+
        '<div class="ireg-f"><label>Comentarios</label><input type="text" id="rn-'+e.id+'" value="'+(sv.comments||'')+'" placeholder="notas..." oninput="markDirty(\''+e.id+'\')"></div>'+
      '</div>'+
      '<button class="btn-ireg-save" onclick="saveReg(\''+e.id+'\')">💾 Guardar</button>'+
      '<div class="ireg-fb" id="sfb-'+e.id+'"></div>'+
    '</div>':'';
  const tmr=e.rs>0?'<button class="btn-tmr" onclick="startTmr('+e.rs+',\''+e.nm.replace(/'/g,"\\'")+'\')">⏱ Descanso '+e.pa+'</button>':'';
  const notaExpand=e.nt?'<div class="ex-nota-expand">💡 '+e.nt+'</div>':'';
  return '<div class="ex-card '+(isDone?'done-card':'')+'" id="card-'+e.id+'">'+
    '<div class="ex-row" onclick="toggle(\''+e.id+'\')">'+
      '<div class="ex-num">'+e.n+'</div>'+
      '<div class="ex-info">'+
        '<div class="ex-name">'+e.nm+'</div>'+
        '<div><span class="ex-badge">'+e.bdg+'</span><span class="reg-saved-dot '+(hasSaved?'visible':'')+'" id="dot-'+e.id+'"></span></div>'+
        '<div class="ex-pause">Pausa: '+e.pa+'</div>'+
        inlineReg+
      '</div>'+
      '<div class="ex-right">'+
        '<a href="'+e.yt+'" target="_blank" rel="noopener" class="btn-vid" onclick="event.stopPropagation()">Ver video</a>'+
        '<div class="ex-chk '+(isDone?'checked':'')+'" id="chk-'+e.id+'" onclick="event.stopPropagation();chkToggle(\''+e.id+'\')">'+
          (isDone?'✓':'')+
        '</div>'+
      '</div>'+
    '</div>'+
    ((tmr||notaExpand)?
      '<div class="ex-body" id="body-'+e.id+'"><div class="ex-body-in">'+tmr+notaExpand+'</div></div>':
      '<div id="body-'+e.id+'"></div>')+
  '</div>';
}`;
